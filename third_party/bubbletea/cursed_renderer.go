...
func (s *cursedRenderer) flush(closing bool) error {
	...
	// A main-screen resize can reflow the frame before it reports its new
	// dimensions. Keep the hidden hardware cursor at the relative frame
	// origin between flushes so the next inline repaint clears that frame,
	// not a reflowed transcript position.
...
func (s *cursedRenderer) insertAbove(str string) error {
	...
	for _, line := range lines {
		...
		// than that in one operation leaves the physical cursor ahead of the
		// renderer's origin and the next footer flush overwrites its tail.
		sb.WriteByte('\r')
		down := h - y - 1
		if down > 0 {
