...
func TestDashboardGeometryUsesSharedColumnsAndFallbacks(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "168x24", width: 168, height: 24, rows: 11, body: 9, padding: 1, input: 123, mapWidth: 40, mapVisible: true, chrome: true},
		{name: "100x30", width: 100, height: 30, rows: 11, body: 9, padding: 1, input: 72, mapWidth: 23, mapVisible: true, chrome: true},
		{name: "100x19", width: 100, height: 19, rows: 7, body: 6, input: 75, mapWidth: 24, mapVisible: true},
		{name: "60x15", width: 60, height: 15, rows: 3, input: 60},
		{name: "10x24", width: 10, height: 24, rows: 4, input: 10},
		{name: "height 20", width: 100, height: 20, rows: 8, body: 6, padding: 1, input: 72, mapWidth: 23, mapVisible: true, chrome: true},
		{name: "height 19", width: 100, height: 19, rows: 7, body: 6, input: 75, mapWidth: 24, mapVisible: true},
		{name: "height 18", width: 100, height: 18, rows: 6, padding: 1, input: 96, chrome: true},
		{name: "width 52", width: 52, height: 24, rows: 11, body: 9, input: 37, mapWidth: 12, mapVisible: true, chrome: true},
		{name: "width 51", width: 51, height: 24, rows: 11, body: 9, input: 36, mapWidth: 12, mapVisible: true, chrome: true},
		{name: "width 50", width: 50, height: 24, rows: 10, body: 9, input: 37, mapWidth: 12, mapVisible: true},
		{name: "width 49", width: 49, height: 24, rows: 10, body: 9, input: 36, mapWidth: 12, mapVisible: true},
		{name: "width 48", width: 48, height: 24, rows: 6, padding: 1, input: 44, chrome: true},
	} {
		t.Run(test.name, func(t *testing.T) {
			got := calculateDashboardGeometry(test.width, test.height, true, currentTheme, "Command > ")
			if got.rows != test.rows || got.bodyHeight != test.body || got.padding != test.padding || got.inputInterior != test.input || got.mapWidth != test.mapWidth || got.mapVisible != test.mapVisible || got.chrome != test.chrome {
...
func TestDashboardGeometryAllocatesWrappedInputRows(t *testing.T) {
	currentTheme := theme{Foreground: "7", Padding: 1}
	for _, test := range []struct {
		name                string
		width, height       int
		mapAvailable        bool
		wantBody, wantInput int
	}{
		{name: "framed map maximum", width: 100, height: 30, mapAvailable: true, wantBody: 9, wantInput: 7},
		{name: "framed map minimum", width: 100, height: 20, mapAvailable: true, wantBody: 6, wantInput: 4},
		{name: "compact map minimum", width: 100, height: 19, mapAvailable: true, wantBody: 6, wantInput: 4},
		{name: "framed no map", width: 100, height: 30, wantInput: 1},
		{name: "compact no map", width: 10, height: 24, wantInput: 1},
	} {
		t.Run(test.name, func(t *testing.T) {
			got := calculateDashboardGeometry(test.width, test.height, test.mapAvailable, currentTheme, "Command > ")
			if got.bodyHeight != test.wantBody || got.inputHeight != test.wantInput {
				t.Fatalf("geometry=%+v", got)
			}
		})
	}
...
func TestFramedDashboardUsesSharedBodyAndExactRows(t *testing.T) {
	...
	if !strings.Contains(rows[0], "Town Square  Exits: north, south") || !strings.Contains(rows[1], "READY | LOG on") || !strings.Contains(rows[2], "L: shield  R: sword  Sp: Fire") || !strings.Contains(rows[3], "Command > draft") {
		t.Fatalf("top rows=%q", rows[:2])
	}
	parts := strings.Split(rows[1], "│")
	if len(parts) != 4 || ansi.StringWidth(parts[1]) != 124 || ansi.StringWidth(parts[2]) != 41 {
		t.Fatalf("body divider positions=%q", rows[1])
	}
	...
	if strings.Contains(strings.Join(rows[4:10], "\n"), "Command >") {
		t.Fatalf("input repeated below input row: %q", rows[4:10])
	}
}
...
func TestCompactDashboardUsesSharedColumnsAtWidthBoundary(t *testing.T) {
	...
	if strings.TrimSpace(plain[0]) != "location" || strings.TrimSpace(plain[1][:38]) != "READY" {
		t.Fatalf("location/status=%q", plain[:2])
	}
	if got := plain[1]; !strings.HasPrefix(got, "READY") || !strings.Contains(got[38:], "@") {
		t.Fatalf("compact status/map row=%q", got)
	}
	if got := plain[2]; !strings.HasPrefix(got, "L: left  R: right") {
		t.Fatalf("compact hands/map row=%q", got)
	}
	if got := plain[3]; !strings.HasPrefix(got, "Command >") {
		t.Fatalf("compact input/map row=%q", got)
	}
}

func TestDashboardHidesMapWhenCompactColumnsDoNotFit(t *testing.T) {
	update := presentation.Update{Location: presentation.Location{Title: "location"}, Hands: presentation.Hands{Left: "left", Right: "right"}}
	rows, visible := dashboardRows(48, 24, update, "status", ">", []string{"map"}, 0, 0, false, theme{Foreground: "7"})
...
func TestDashboardHidesRowsInPriorityOrder(t *testing.T) {
	...
	for _, test := range []struct {
		...
		view := renderDashboard(48, test.height, update, "status", ">", []string{"map"}, 0, 0, false, theme{Foreground: "7"})
...
func TestDashboardWithoutMapRetainsSixRowFrame(t *testing.T) {
	...
	rows := strings.Split(ansi.Strip(view), "\n")
	if len(rows) != 6 || !strings.HasPrefix(rows[0], "╭") || !strings.Contains(rows[0], "location") || !strings.Contains(rows[1], "status") || !strings.Contains(rows[2], "L: -  R: -") || !strings.Contains(rows[4], ">") {
...
func TestFramedDashboardPreservesHorizontalPadding(t *testing.T) {
	...
	if len(parts) != 4 || !strings.HasPrefix(parts[1], "  ") || !strings.HasSuffix(parts[2], "  ") {
...
func TestMapNavigationPrefixesStatusAndColorsMapBoundary(t *testing.T) {
	...
	if rows[0] != wantTop || !strings.Contains(rows[1], "38;5;214") {
		t.Fatalf("map boundary was not highlighted: %q", rows[1])
	}
}
...
func TestCompactRowsApplyThemeByRole(t *testing.T) {
	...
	rows, visible := dashboardRows(50, 24, presentation.Update{Location: presentation.Location{Title: "location"}}, "READY", "Command > first\n          second", []string{"@"}, 0, 0, false, currentTheme)
	if !visible || !strings.Contains(rows[0], "38;5;231") || strings.Contains(rows[0], "48;5;220") {
		t.Fatalf("location style=%q", rows[0])
	}
	if !strings.Contains(rows[1], "38;5;16") || !strings.Contains(rows[1], "48;5;220") || strings.Contains(rows[2], "48;5;220") || !strings.Contains(rows[3], "48;5;220") || !strings.Contains(rows[4], "48;5;220") {
		t.Fatalf("status/hands/input style=%q", rows[1:5])
	}
}

func TestDashboardTruncatesStatusAndHandsWithinChatColumn(t *testing.T) {
	useANSI256(t)
	currentTheme := theme{Foreground: "231", Border: "39", TitleBar: "214", StatusBar: "16", StatusBarBg: "220", Padding: 1}
	update := presentation.Update{Hands: presentation.Hands{
		Left:          "\x1b[31m界界界界界界界界界界界界界界界界界界界界界界界界界界界界界界界界界界界界界界\x1b[0m",
		Right:         "右右右右右右右右右右右右右右右右右右右右",
		PreparedSpell: "火火火火火火火火火火火火火火火火火火火火",
	}}
	status := "READY | " + strings.Repeat("🙂", 50)
	rows := strings.Split(renderDashboard(100, 30, update, status, "Command > draft", []string{"@"}, 0, 0, false, currentTheme), "\n")
	geometry := calculateDashboardGeometry(100, 30, true, currentTheme, "")
	for _, index := range []int{1, 2} {
		parts := strings.Split(ansi.Strip(rows[index]), "│")
		if len(parts) != 4 || ansi.StringWidth(parts[1]) != geometry.padding+geometry.inputInterior || !strings.Contains(parts[1], "…") || strings.Contains(parts[2], "界") || strings.Contains(parts[2], "右") || strings.Contains(parts[2], "火") {
			t.Fatalf("row %d escaped chat column: %q", index, ansi.Strip(rows[index]))
		}
	}
	if !strings.Contains(rows[1], "48;5;220") || strings.Contains(rows[2], "48;5;220") {
		t.Fatalf("status background crossed role boundary: %q", rows[1:3])
	}

	compactRows := strings.Split(renderDashboard(49, 24, update, status, "Command > draft", []string{"@"}, 0, 0, false, currentTheme), "\n")
	for _, index := range []int{1, 2} {
		plain := ansi.Strip(compactRows[index])
		chat := ansi.Cut(plain, 0, 36)
		mapText := ansi.Cut(plain, 37, 49)
		if ansi.StringWidth(chat) != 36 || !strings.Contains(chat, "…") || strings.ContainsAny(mapText, "界右火🙂") {
			t.Fatalf("compact row %d escaped chat column: %q", index, plain)
		}
	}
}
